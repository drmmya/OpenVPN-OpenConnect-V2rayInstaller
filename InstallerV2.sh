...
<div class="card" style="margin-bottom:18px">
  <div class="toolbar" style="margin-bottom:0">
    <div>
      ...
      <div id="ocurl" style="font-size:18px;font-weight:700;word-break:break-all">https://<?=esc((function(){ $e=[]; if(is_file('/etc/vpn.env')) foreach(file('/etc/vpn.env', FILE_IGNORE_NEW_LINES|FILE_SKIP_EMPTY_LINES) as $l){ if(strpos($l,'=')!==false){[$k,$v]=explode('=',$l,2); $e[trim($k)]=trim($v);} } return $e['OC_HOST'] ?? 'oc.mustakimshop.online'; })())?>:443</div>
    </div>
  ...
  "inbounds": [
    {
      ...
      "streamSettings": {
        ...
        "tlsSettings": {
          ...
          "certificates": [
            {
              "certificateFile": "${XRAY_SSL_DIR}/xray.crt",
              "keyFile": "${XRAY_SSL_DIR}/xray.key"
            }
          ]
        }
      },
...
echo "[18/22] Installing HAProxy 443 SNI router..."
...
frontend vpn_443
    ...
    use_backend xray_backend if { req.ssl_sni -i ${V2_HOST} }
    use_backend ocserv_backend if { req.ssl_sni -i ${OC_HOST} }

    default_backend ocserv_backend

backend ocserv_backend
    mode tcp
    server ocserv 127.0.0.1:4443
...
function xray_active_connections(){
    $count = xcmd("ss -tn state established \'( sport = :8444 or dport = :8444 )\' | tail -n +2 | wc -l");
    return (int)$count;
}
...
$link = $uuid ? "vless://".$uuid."@".$v2host.":443?type=tcp&security=tls&sni=".$v2host."&allowInsecure=1#Xray-".$v2host : '';
...
<div class="grid">
  ...
  <div class="card"><div class="muted">Public port</div><div class="kpi">443</div></div>
  <div class="card"><div class="muted">Backend port</div><div class="kpi">8444</div></div>
</div>
...
<div class="card" style="margin-top:18px">
  <div class="toolbar">
    <div>
      ...
      <div class="small">Public host uses HAProxy SNI: <strong><?=esc($v2host)?>:443</strong></div>
    </div>
...
echo "Xray checks:"
echo "  ss -tn sport = :8444 | grep ESTAB | wc -l"
...
echo "   3) OpenConnect : https://${OC_HOST}:443"
echo "   4) Xray/V2Ray  : ${V2_HOST}:443"
echo
echo "🧩 SNI Routing:"
echo "   ${OC_HOST}:443  -> OpenConnect backend 127.0.0.1:4443"
echo "   ${V2_HOST}:443  -> Xray/V2Ray backend 127.0.0.1:8444"
echo
...
echo "   vless://${XRAY_UUID}@${V2_HOST}:443?type=tcp&security=tls&sni=${V2_HOST}&allowInsecure=1#Xray-${V2_HOST}"
echo
echo "🧪 Service Status:"
systemctl is-active --quiet apache2 && echo "   Apache Panel : running" || echo "   Apache Panel : not running"
systemctl is-active --quiet haproxy && echo "   HAProxy      : running" || echo "   HAProxy      : not running"
...
echo
echo "⚠️ DNS Required:"
echo "   oc.${DOMAIN_NAME}   A   ${SERVER_ADDR}"
echo "   v2.${DOMAIN_NAME}   A   ${SERVER_ADDR}"
echo "   ovpn.${DOMAIN_NAME} A   ${SERVER_ADDR}"
echo
