...
function run_bg_control($args,$proto){ $safe=preg_replace('/[^a-z0-9_-]/i','',$proto); $log='/var/log/vpn-panel-install-'.$safe.'.log'; if((file_exists($log)&&!is_writable($log))||(!file_exists($log)&&!is_writable(dirname($log)))) $log=DATA_DIR.'/vpn-panel-install-'.$safe.'.log'; $cmd='nohup sudo -n /usr/local/bin/vpn-control.sh '.$args.' > '.escapeshellarg($log).' 2>&1 < /dev/null & echo $!'; exec($cmd,$out,$code); return [$code, trim($out[0]??''), $log]; }
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if($action==='quick_install' && in_array($proto,['openvpn','openconnect','v2ray'],true)){
    ...
    if(!$err) $c===0?$msg='Install started in background'.($pid?' (PID '.$pid.')':'').'. Check log: '.$log.'. Refresh after it finishes.':$err='Failed to start install job. Check sudo permission and Apache error log.';
  }
}
$bw=vps_bandwidth();
