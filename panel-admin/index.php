...
function run_bg_control($args,$proto){
  ...
  exec($cmd,$out,$code); $pid=trim($out[0]??''); if($pid!=='') @file_put_contents(DATA_DIR.'/vpn-panel-install-'.$safe.'.pid',$pid); return [$code,$pid,$log];
}
...
<div class="panel-banner"><div class="toolbar"><div><h2 class="section-title">Modern Live VPN Dashboard</h2><div class="small"><span class="live-dot"></span> Cards update every 5 seconds in background.</div></div><a class="btn" href="system_control.php">System Control</a></div></div>
<div class="hero-grid">
