...
function run_bg_control($args,$proto){
  ...
  exec($cmd,$out,$code);
  $pid=trim($out[0]??'');
  if($pid!=='') @file_put_contents(DATA_DIR.'/vpn-panel-install-'.$safe.'.pid',$pid);
  return [$code, $pid, $log];
}
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if(in_array($proto,['openvpn','openconnect','v2ray'],true)){
    ...
    if($action==='install'){
      ...
      if(!$err) $c===0?$msg='Install started. Mini live log is showing inside the protocol card'.($pid?' (PID '.$pid.')':'').'.':$err='Failed to start install job. Check sudo permission and Apache error log.';
    }
...
<?php foreach($items as $it): $is=installed($it['key']); $running=svc_on($it['proto']); ?>
  <div class="card gradient-card protocol-card <?=esc($it['class'])?>" data-proto-card="<?=esc($it['proto'])?>">
    ...
    <form method="post" class="install-form" data-install-proto="<?=esc($it['proto'])?>">
      ...
      <?php endif; ?><br><br>
      <?php if(!$is): ?><button class="btn green install-btn" name="action" value="install">Install <?=esc($it['name'])?></button><?php else: ?><button class="btn" name="action" value="change_port">Change Port</button><?php endif; ?>
    </form>
...
function badgeClass(status){ if(status === 'running') return 'badge yellow'; if(status === 'success') return 'badge green'; if(status === 'failed') return 'badge red'; return 'badge'; }
...
async function loadMiniLog(proto){
  try{
    ...
    if(!box || !badge) return;
    if(!data.ok){ box.textContent = data.error || 'Unable to read log'; return; }
...
document.querySelectorAll('.install-form').forEach(form=>form.addEventListener('submit',()=>{ const proto=form.dataset.installProto; currentLogProto=proto; const b=form.querySelector('.install-btn'); if(b){ b.disabled=true; b.textContent='Starting...'; } setTimeout(refreshAll,800); }));
refreshAll(); logTimer=setInterval(refreshAll,3000);
</script>
<?php render_footer(); ?>
