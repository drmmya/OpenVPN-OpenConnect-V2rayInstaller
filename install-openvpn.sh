...
apt-get install -y openvpn easy-rsa apache2 php libapache2-mod-php php-sqlite3 php-cli sqlite3 curl openssl ca-certificates acl netcat-openbsd iptables >/dev/null
...
#!/usr/bin/env bash
set -euo pipefail
USER_NAME="\${1:?username required}"; SERVER_ADDR="\${2:?server addr required}"; OUT_DIR="$DOWNLOAD_DIR"; PKI_DIR="$PKI_DIR"; mkdir -p "\$OUT_DIR"
...
auth-user-pass
auth-nocache
remote \${SERVER_ADDR} ${UDP_PORT} udp
remote \${SERVER_ADDR} ${TCP_PORT} tcp-client
remote-random
...
#!/usr/bin/env bash
set -euo pipefail
DB="$DB_FILE"; SERVER_ADDR="\${SERVER_ADDR_OVERRIDE:-\$(curl -4 -fsSL https://api.ipify.org 2>/dev/null || hostname -I | awk '{print \$1}')}"; cmd="\${1:-}"; user="\${2:-}"; pass="\${3:-}"; esc(){ printf "%s" "\$1"|sed "s/'/''/g"; }
case "\$cmd" in
add) [[ -n "\$user" && -n "\$pass" ]] || exit 1; hash="\$(php -r 'echo password_hash(\$argv[1], PASSWORD_DEFAULT);' "\$pass")"; sqlite3 "\$DB" "INSERT INTO ovpn_users(username,password_hash,blocked) VALUES('\$(esc "\$user")','\$(esc "\$hash")',0);"; /usr/local/bin/ovpn-make-profile.sh "\$user" "\$SERVER_ADDR"; echo "User added: \$user";;
update) [[ -n "\$user" && -n "\$pass" ]] || exit 1; hash="\$(php -r 'echo password_hash(\$argv[1], PASSWORD_DEFAULT);' "\$pass")"; sqlite3 "\$DB" "UPDATE ovpn_users SET password_hash='\$(esc "\$hash")',updated_at=CURRENT_TIMESTAMP WHERE username='\$(esc "\$user")';"; /usr/local/bin/ovpn-make-profile.sh "\$user" "\$SERVER_ADDR"; echo "User updated: \$user";;
delete) [[ -n "\$user" ]] || exit 1; sqlite3 "\$DB" "DELETE FROM ovpn_users WHERE username='\$(esc "\$user")';"; rm -f "$DOWNLOAD_DIR/\$user.ovpn"; echo "User deleted: \$user";;
...
function ovpn_users(){ $res=db()->query('SELECT id,username,blocked,created_at,updated_at FROM ovpn_users ORDER BY id DESC'); $rows=[]; while($res && $row=$res->fetchArray(SQLITE3_ASSOC)) $rows[]=$row; return $rows; }
function ovpn_logs($limit=80){ $st=db()->prepare("SELECT * FROM ovpn_events WHERE event_type='connect' ORDER BY id DESC LIMIT :l"); $st->bindValue(':l',(int)$limit,SQLITE3_INTEGER); $r=$st->execute(); $rows=[]; while($r && $row=$r->fetchArray(SQLITE3_ASSOC)) $rows[]=$row; return $rows; }
...
function last_gui_for_user($u){ $st=db()->prepare("SELECT gui_version FROM ovpn_events WHERE username=:u AND COALESCE(gui_version,'')<>'' ORDER BY id DESC LIMIT 1"); $st->bindValue(':u',$u,SQLITE3_TEXT); $r=$st->execute(); $row=$r?$r->fetchArray(SQLITE3_ASSOC):false; return $row['gui_version']??''; }
...
$msg='';$err=''; if($_SERVER['REQUEST_METHOD']==='POST'){ $a=$_POST['action']??''; $u=trim($_POST['username']??$_POST['edit_username']??''); $p=$_POST['password']??$_POST['edit_password']??''; if($u!==''&&$p!==''){ [$code,$out]=cli('/usr/local/bin/ovpn-user-manage.sh '.($a==='edit'?'update':'add').' '.escapeshellarg($u).' '.escapeshellarg($p)); $code===0?$msg=$out:$err=$out; } }
foreach(['delete'=>'delete','block'=>'block','unblock'=>'unblock'] as $q=>$cmd){ if(isset($_GET[$q])){ cli('/usr/local/bin/ovpn-user-manage.sh '.$cmd.' '.escapeshellarg(trim($_GET[$q]))); header('Location: openvpn.php'); exit; } }
...
<div class="grid"><div class="card"><div class="muted">Total users</div><div class="kpi"><?=count($users)?></div></div><div class="card"><div class="muted">Active connections</div><div class="kpi"><?=count($active)?></div></div><div class="card"><div class="muted">UDP Port</div><div class="kpi"><?=esc(cfgv('OVPN_UDP_PORT','1194'))?></div></div><div class="card"><div class="muted">TCP Port</div><div class="kpi"><?=esc(cfgv('OVPN_TCP_PORT','8443'))?></div></div></div>
...
<div class="card" style="margin-top:18px"><h2 class="section-title">OpenVPN Users</h2><div class="table-wrap"><table style="min-width:1000px"><tr><th>Username</th><th>Status</th><th>Active Devices</th><th>Edit Password</th><th>Actions</th></tr><?php foreach($users as $u): $name=$u['username']; ?><tr><td><strong><?=esc($name)?></strong></td><td><?=((int)$u['blocked']===1)?'<span class="badge red">Blocked</span>':'<span class="badge green">Active</span>'?></td><td><span class="badge"><?=esc($activeBy[$name]??0)?> connected</span></td><td><form method="post" class="actions"><input type="hidden" name="action" value="edit"><input type="hidden" name="edit_username" value="<?=esc($name)?>"><input name="edit_password" placeholder="New password" required><button class="btn">Update</button></form></td><td class="actions"><a class="btn green" href="download.php?u=<?=urlencode($name)?>">Download</a><?php if((int)$u['blocked']===1): ?><a class="btn yellow" href="?unblock=<?=urlencode($name)?>">Unblock</a><?php else: ?><a class="btn red" href="?block=<?=urlencode($name)?>">Block</a><?php endif; ?><a class="btn red" href="?delete=<?=urlencode($name)?>" onclick="return confirm('Delete this user?')">Delete</a></td></tr><?php endforeach; ?></table></div></div>
<div class="card" style="margin-top:18px"><div class="toolbar"><h2 class="section-title">OpenVPN Active Connected Devices</h2><span class="badge green"><?=count($active)?> active</span></div><div class="table-wrap"><table><tr><th>User</th><th>Protocol</th><th>GUI Version</th><th>Real IP</th><th>Virtual IP</th><th>Since</th><th>Download</th><th>Upload</th></tr><?php if(!$active): ?><tr><td colspan="8" class="empty">No active OpenVPN devices.</td></tr><?php else: foreach($active as $c): $u=$c['username']?:$c['common_name']; ?><tr><td><strong><?=esc($u)?></strong></td><td><span class="badge"><?=esc($c['source'])?></span></td><td class="small"><?=esc($c['gui_version']?:'-')?></td><td><?=esc($c['real_address'])?></td><td><?=esc($c['virtual_address']?:'-')?></td><td><?=esc($c['connected_since'])?></td><td><?=esc(human_bytes($c['bytes_received']))?></td><td><?=esc(human_bytes($c['bytes_sent']))?></td></tr><?php endforeach; endif; ?></table></div></div>
<div class="card" style="margin-top:18px"><h2 class="section-title">Recent OpenVPN logs</h2><div class="table-wrap"><table><tr><th>Time</th><th>User</th><th>IP</th><th>Virtual IP</th><th>GUI Version</th></tr><?php foreach($logs as $r): ?><tr><td><?=esc($r['event_time'])?></td><td><?=esc($r['username']?:$r['common_name'])?></td><td><?=esc($r['real_ip'])?></td><td><?=esc($r['virtual_ip'])?></td><td class="small"><?=esc($r['gui_version']?:'-')?></td></tr><?php endforeach; ?></table></div></div><?php render_footer(); ?>
PHP
cat >"$APP_DIR/download.php" <<PHP
<?php require __DIR__.'/config.php'; require_login(); \$u=basename(\$_GET['u']??''); \$p=DOWNLOAD_DIR.'/'.\$u.'.ovpn'; if(!is_file(\$p)){http_response_code(404);exit('Profile not found');} header('Content-Type: application/octet-stream'); header('Content-Disposition: attachment; filename="'.\$u.'.ovpn"'); readfile(\$p);
PHP

chown -R www-data:www-data "$APP_DIR"; chmod -R 755 "$APP_DIR"; chmod -R 775 "$DATA_DIR" "$DOWNLOAD_DIR"; chmod 664 "$DB_FILE"
