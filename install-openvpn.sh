...
[Unit]
Description=Apply iptables rules for VPN Panel
...
#!/usr/bin/env php
<?php
$db = new SQLite3('/var/www/html/panel-admin/data/ovpn.sqlite');
$db->busyTimeout(5000);
...
auth-user-pass
auth-nocache
remote ${SERVER_ADDR} ${UDP_PORT:-1194} udp
remote ${OVPN_HOST:-$SERVER_ADDR} ${TCP_PORT:-8443} tcp-client
remote-random
...
#!/usr/bin/env bash
set -euo pipefail
DB="/var/www/html/panel-admin/data/ovpn.sqlite"
...
case "$cmd" in
  ...
  delete)
    ...
    rm -f "/var/www/html/panel-admin/downloads/$user.ovpn"
...
# Public panel URL alias: /vpn-panel -> /var/www/html/panel-admin
cat >/etc/apache2/conf-available/vpn-panel.conf <<APACHE
...
echo
echo "Done."
echo "Panel URL: http://${SERVER_ADDR}/vpn-panel/"
