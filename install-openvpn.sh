...
apt-get install -y openvpn easy-rsa apache2 php libapache2-mod-php php-sqlite3 php-cli sqlite3 curl openssl ca-certificates acl netcat-openbsd iptables iproute2 python3 >/dev/null
...
#!/usr/bin/env bash
set -euo pipefail
DB="$DB_FILE"; SERVER_ADDR="\${SERVER_ADDR_OVERRIDE:-\$(curl -4 -fsSL https://api.ipify.org 2>/dev/null || hostname -I | awk '{print \$1}')}"; cmd="\${1:-}"; user="\${2:-}"; pass="\${3:-}"; esc(){ printf "%s" "\$1"|sed "s/'/''/g"; }; valid_user(){ [[ "\${1:-}" =~ ^[A-Za-z0-9._-]{3,32}$ ]]; }; [[ -z "\$user" || "\$cmd" == "" ]] || valid_user "\$user" || { echo "Invalid username. Use 3-32 chars: A-Z a-z 0-9 . _ -" >&2; exit 1; }
case "\$cmd" in
...
$msg='';$err=''; if($_SERVER['REQUEST_METHOD']==='POST'){ $a=$_POST['action']??''; $u=trim($_POST['username']??$_POST['edit_username']??''); $p=$_POST['password']??$_POST['edit_password']??''; if(in_array($a,['add','edit'],true)){ if(!valid_username($u)){$err='Invalid username. Use 3-32 chars: A-Z a-z 0-9 . _ -';} elseif($p===''){$err='Password required';} else { [$code,$out]=cli('/usr/local/bin/ovpn-user-manage.sh '.($a==='edit'?'update':'add').' '.escapeshellarg($u).' '.escapeshellarg($p)); $code===0?$msg=$out:$err=$out; } } elseif(in_array($a,['delete','block','unblock'],true)){ if(!valid_username($u)){$err='Invalid username';} else { [$code,$out]=cli('/usr/local/bin/ovpn-user-manage.sh '.$a.' '.escapeshellarg($u)); $code===0?$msg=$out:$err=$out; } } }
...
chown -R root:www-data "$APP_DIR"; find "$APP_DIR" -type d -exec chmod 755 {} \; ; find "$APP_DIR" -type f -exec chmod 644 {} \; ; chown -R www-data:www-data "$DATA_DIR" "$DOWNLOAD_DIR"; chmod -R 775 "$DATA_DIR" "$DOWNLOAD_DIR"; chmod 664 "$DB_FILE"
