...
<div class="card" style="margin-top:18px">
  ...
  <div class="small" style="margin-top:12px">OpenConnect URL: https://<?=esc($_SERVER['SERVER_ADDR'] ?: $_SERVER['SERVER_NAME'])?>:443</div>
</div>
...
$active = trim(shell_exec("ss -Htn state established '( sport = :4443 or dport = :4443 )' 2>/dev/null | awk '{print $5}' | sed 's/::ffff://g' | sed 's/^\\[//;s/\\]//' | sed 's/:.*$//' | grep -E '^[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+$' | sort -u | wc -l"));
if($active === '') $active = '0';
...
<div class="card" style="margin-top:18px">
  <div class="toolbar" style="align-items:center">
    <div>
      ...
      <div class="small">NekoBox বা latest v2rayNG app-এ এই config import করুন। Port: 4443</div>
    </div>
    <span class="badge green">4443</span>
  </div>
  ...
  <div class="small" style="margin-top:12px">
    Address: <?=esc($serverIp)?> | Port: 4443 | Protocol: VLESS | Network: TCP | Security: none
  </div>
</div>
