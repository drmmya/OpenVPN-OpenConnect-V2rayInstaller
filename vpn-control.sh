...
ensure_free(){
  ...
  if [[ "$p" != "$current" ]] && port_in_use "$p"; then fail "Port $p already used. Choose another port."; fi
}
...
svc_stop(){ case "$1" in openvpn) systemctl stop openvpn-server@server-udp openvpn-server@server-tcp 2>/dev/null || true; verify_state openvpn inactive ;; openconnect) systemctl stop ocserv; verify_state openconnect inactive ;; v2ray|xray) systemctl stop xray; verify_state v2ray inactive ;; *) fail "Unknown protocol: $1" ;; esac; }
svc_restart(){ case "$1" in openvpn) systemctl restart openvpn-server@server-udp openvpn-server@server-tcp; verify_state openvpn active ;; openconnect) systemctl restart ocserv; verify_state openconnect active ;; v2ray|xray) systemctl restart xray; verify_state v2ray active ;; *) fail "Unknown protocol: $1" ;; esac; }
...
change_openconnect_port(){
  local port="$1" old
  old="$(get_conf OC_PORT 443)"; ensure_free "$port" "$old"
...
change_v2ray_port(){
  ...
  old="$(get_conf V2_PORT 4443)"; ensure_free "$port" "$old"
...
install_protocol(){
  ...
  case "$proto" in
    openvpn)
      ...
      set_conf OPENVPN 1; set_conf OVPN_UDP_PORT "$udp"; set_conf OVPN_TCP_PORT "$tcp"
      write_ovpn_profile_generator; regen_ovpn_profiles
...
case "$ACTION" in
  ...
  port-check) is_port "$PROTO" || fail "Invalid port: $PROTO"; if port_in_use "$PROTO"; then echo "USED"; exit 2; else echo "FREE"; fi ;;
