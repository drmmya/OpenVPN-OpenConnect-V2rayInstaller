...
cleanup_old_install(){
  ...
  # Load old port values if an older modular install exists, so old firewall rules can be removed.
  ...
  # Remove old panel/app paths and generated protocol data from both old and new layouts.
  rm -rf /var/www/html/ovpn-admin /var/www/html/panel-admin
  rm -rf /etc/openvpn/pki-webadmin /root/easy-rsa
  rm -f /etc/openvpn/server/server-udp.conf /etc/openvpn/server/server-tcp.conf
  rm -f /etc/ocserv/ocserv.conf /etc/ocserv/ocpasswd
  ...
  rm -f /usr/local/bin/oc-user-manage.sh /usr/local/bin/oc-event-log.sh
  ...
  # Best-effort cleanup of known old firewall rules. Ignore failures safely.
  ...
  for net in 10.8.0.0/24 10.9.0.0/24 10.20.30.0/24; do
    while iptables -t nat -D POSTROUTING -s "$net" -j MASQUERADE 2>/dev/null; do :; done
    while iptables -D FORWARD -s "$net" -j ACCEPT 2>/dev/null; do :; done
    while iptables -D FORWARD -d "$net" -m state --state ESTABLISHED,RELATED -j ACCEPT 2>/dev/null; do :; done
  done
